...
function asBoardEntry(snap: QueryDocumentSnapshot<DocumentData> | { id: string; data(): DocumentData }): BoardEntryDoc {
  ...
  return {
    ...
    name: data.name,
    status: data.status,
    bestAttemptNo: data.bestAttemptNo,
    metrics: data.metrics,
...
async function getUniqueCode(makeCode: () => string, collectionName: 'joinCodes' | 'teacherCodes' | 'organizerInvites'): Promise<string> {
...
export function createFirestoreApi(): CompetitionApi {
  ...
  async function readAttempts(path: ParticipantPath): Promise<AttemptDoc[]> {
    ...
    return snaps.docs.map(asAttempt).sort((a, b) => a.attemptNo - b.attemptNo)
  }
  ...
  return {
    ...
    async submitAttempt(path, metrics) {
      try {
        const uid = await ensureUser()
        ...
        if (attempts.length >= event.maxAttempts) throw new Error('NO_ATTEMPTS_LEFT')
        const attemptNo = Math.max(0, ...attempts.map((x) => x.attemptNo)) + 1
        if (attemptNo > event.maxAttempts) throw new Error('NO_ATTEMPTS_LEFT')

        await setDoc(doc(attemptsCollection(path), `${path.participantId}_${attemptNo}`), {
          attemptNo,
          metrics,
          submittedAt: serverTimestamp(),
        })

        const currentBoard = await getDoc(boardRef(path))
        const current = currentBoard.exists() ? asBoardEntry(currentBoard).metrics : null
        if (!currentBoard.exists() || isBetter(metrics, current, event.scoringVersion)) {
        ...
        return { attemptNo, remaining: event.maxAttempts - attemptNo }
      } catch (error) {
        return mapError(error)
      }
    },
    ...
    async getLeaderboard(joinCode, opts) {
      try {
        ...
        return snaps.docs
          ...
          .map<LeaderboardRow>((entry, index) => ({
            rank: index + 1,
            publicId: entry.publicId,
            name: entry.name,
            status: entry.status,
            score: computeScore(entry.metrics, info.event.scoringVersion),
            metrics: entry.metrics,
            attemptsUsed: entry.bestAttemptNo,
          }))
      } catch (error) {
        return mapError(error)
      }
    ...
    async listMySchools() {
      try {
        ...
        const role = roleSnap.data() as TeacherRoleDoc
        const paths = role.schoolPaths ?? []
    ...
    async importSchools(eventId, rows) {
      try {
        ...
        for (const row of rows) {
          ...
          if (classSnaps.docs.some((snap) => String(snap.data().name).trim() === className) || classes.some((c) => c.schoolId === school.id && c.name === className)) {
    ...
    async createOrganizerInvite() {
      try {
        const uid = await ensureUser()
        const code = await getUniqueCode(newInviteCode, 'organizerInvites')
        await setDoc(doc(db, 'organizerInvites', code), {
          createdBy: uid,
          usedBy: null,
          createdAt: serverTimestamp(),
        })
    ...
    async redeemOrganizerInvite(code) {
      try {
        const uid = await ensureUser()
        const normalized = normalizeCode(code)
        const inviteSnap = await getDoc(doc(db, 'organizerInvites', normalized))
        if (!inviteSnap.exists() || inviteSnap.data().usedBy !== null) throw new Error('INVITE_NOT_FOUND')
        const batch = writeBatch(db)
        batch.update(doc(db, 'organizerInvites', normalized), { usedBy: uid })
        batch.set(doc(db, 'roles', uid), {
          role: 'organizer',
